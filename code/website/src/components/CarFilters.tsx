...
const inputClass = "border border-foreground bg-background focus:border-foreground focus:ring-0 focus:ring-offset-0 focus:outline-none focus-visible:outline-none focus-visible:ring-0 focus-visible:ring-offset-0 hover:border-foreground transition-none leading-6";
...
const CustomDropdown = ({ value, onChange, options, placeholder, disabled = false }: {
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      {isOpen && (
        <div className="absolute z-50 w-full mt-[-1px] bg-background border border-foreground rounded-md shadow-lg max-h-60 overflow-auto">
          {options.filter(option => option !== placeholder).map((option) => (
            <Button
              key={option}
              variant="dropdown"
              type="button"
              className="w-full justify-start rounded-none border-x-0 border-t-0 border-b border-foreground last:border-b-0"
              onClick={() => {
                onChange(option);
                setIsOpen(false);
              }}
...
const CarFilters = ({ options, onFilterChange }: CarFiltersProps) => {
  ...
  }, [debouncedSearch, selectedMerk, minPrijs, maxPrijs, minBouwjaar, maxBouwjaar, selectedBrandstofType, selectedTransmissie, sortBy, sortOrder]);
