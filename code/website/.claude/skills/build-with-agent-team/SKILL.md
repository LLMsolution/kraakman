...
**ARCHON-FIRST:** This skill uses Archon MCP as the primary system for project management, task tracking, research, and contract storage. The lead agent manages ALL Archon interactions — spawned agents communicate via SendMessage, and the lead updates Archon on their behalf.
...
### RAG Research (Before Spawning)

After reading the plan, research the tech stack via Archon RAG. This makes agents smarter before they start — they get relevant docs and examples alongside the plan.
...
2. **Search for each key technology** (2-5 keywords per query):
   ```
   rag_search_knowledge_base(query="FastAPI SSE streaming", match_count=3)
   rag_search_knowledge_base(query="React server components", match_count=3)
   rag_search_code_examples(query="SQLite async Python", match_count=3)
   ```

3. **If a specific doc source is relevant**, filter by source_id:
   ```
   rag_search_knowledge_base(query="routing middleware", source_id="src_abc123", match_count=5)
   ```

4. **Collect findings** — include relevant snippets in agent spawn prompts (Step 5).

## Step 2: Archon Bootstrap

Before spawning agents, set up Archon as the central coordination system.

### Create Project

```
manage_project("create", title="Agent Team Build: {plan-name}", description="Multi-agent build from plan: {plan-path}")
```

Save the returned `project_id` — all tasks and documents reference it.

### Store Plan as Document

```
manage_document("create", project_id="...", title="Build Plan", document_type="plan", content="{plan content}")
```

### Create Tasks

Create Archon tasks for each agent's work. Use `task_order` for dependency ordering (higher = earlier in sequence). This replaces plain-text task lists.

**Upstream agents get higher task_order (executed first):**

```
# Database/Schema agent — highest priority, others depend on this
manage_task("create", project_id="...", title="Database: Design schema + publish contract", task_order=100, description="...")

# Backend agent — depends on DB contract
manage_task("create", project_id="...", title="Backend: Implement API + publish contract", task_order=80, description="...")

# Frontend agent — depends on API contract
manage_task("create", project_id="...", title="Frontend: Build UI to API contract", task_order=60, description="...")

# Integration tasks
manage_task("create", project_id="...", title="Lead: Contract verification", task_order=90, description="Verify each contract before forwarding downstream")
manage_task("create", project_id="...", title="Lead: End-to-end validation", task_order=10, description="Full system validation after all agents complete")
```

**Task granularity:** Create one task per major deliverable per agent. For a 3-agent team, expect 5-8 tasks total. Don't over-subdivide — agents handle their own subtask decomposition.
...
## Step 5: Contract-First Spawning
...
**Archon tracking at each step:**
- When spawning an agent: `manage_task("update", task_id="...", status="doing")`
- When contract is received: note in task description
- When contract is verified and forwarded: `manage_task("update", task_id="...", status="review")` for the contract verification task
...
3. **Store the verified contract as an Archon document:**
   ```
   manage_document("create", project_id="...", title="Verified Contract: Backend API", document_type="contract", content="{exact contract}")
   ```
4. **Forward it to consuming agents** with explicit instructions: "Build to this contract exactly. Do not deviate."
...
[Include relevant research findings from Step 1 — docs, code examples, patterns]
...
**Archon: Store each verified contract as a document:**
```
manage_document("create", project_id="...", title="Verified Contract: {agent} → {consumer}", document_type="contract", content="{exact contract}")
```
...
**Archon: Update task status as agents progress:**
- Agent starts implementation → `manage_task("update", task_id="...", status="doing")`
- Agent reports progress → update task description with notes if needed
...
**Archon: Mark contract verification task as done:**
```
...
**Archon: Move agent tasks to review:**
```
...
**Anti-pattern 4: Skipping Archon** (no audit trail)
```
Lead tracks everything in-context only
Context compresses → loses track of contracts, task status, who owns what ❌
```

**Good pattern: Contract-first with lead relay**
```
Backend publishes contract → Lead verifies → Lead stores in Archon → Lead forwards to frontend
...
5. **Missing dependencies**: Agent B waits on Agent A forever → Track blockers actively via Archon task status
...
11. **Skipping Archon updates**: Lead forgets to update task status → loses track of progress. Update Archon at every milestone
...
7. **All Archon tasks are marked `done`**
8. **Validation results are stored as an Archon document**
...
4. Tests: Write tests for your domain (happy path, edge cases, error handling)
...
1. **Store validation results:**
   ```
   manage_document("create", project_id="...", title="Validation Report", document_type="report", content="{validation results, pass/fail per check}")
   ```

2. **Finalize all tasks:**
   ```
   manage_task("update", task_id="...", status="done")  # for each remaining task
   ```

3. **Update project status:**
   ```
   manage_project("update", project_id="...", description="Build complete. All validations passed.")
   ```
...
2. **Research tech stack via Archon RAG** — search knowledge base and code examples for each key technology
3. **Create Archon project, tasks, and store plan as document**
4. Determine team size (use `$ARGUMENTS[1]` if provided, otherwise decide)
5. Define agent roles, ownership, **cross-cutting concern assignments**, and **validation requirements for each**
6. **Map the contract chain** — which agent produces interfaces that others consume?
7. Enter Delegate Mode
8. **Spawn upstream agents first** — their first task is publishing their contract. Include RAG research findings in spawn prompts.
9. **Receive and verify each contract** — check for ambiguities, exact URLs, response shapes
10. **Store verified contracts as Archon documents**
11. **Forward verified contracts to downstream agents** — include in their spawn prompt
12. Spawn downstream agents with verified contracts + their validation checklist
13. **Update Archon task status as agents progress** (doing → review → done)
14. **Run contract diff before integration** — compare backend's curl commands vs frontend's fetch URLs
15. When all agents return, run **end-to-end validation yourself** (start both servers, use agent-browser for UI testing)
16. If validation fails, re-spawn the relevant agent with the specific issue
17. **Store validation results as Archon document and finalize all tasks**
